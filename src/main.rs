...
pub use zeroclaw::{
    ...
    MigrateCommands, PeripheralCommands, ServiceCommands, SkillCommands, StationCommands,
};
...
Examples:
  ...
  zeroclaw daemon --host 127.0.0.1  # localhost only")]
    ...
    /// Start or inspect NeoHUman Station
    Station {
        #[command(subcommand)]
        station_command: Option<zeroclaw::StationCommands>,
    },
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Station { station_command } => match station_command {
            Some(zeroclaw::StationCommands::Status) => {
                let report = appliance::initialize_station(&config).await?;
                println!("{}", appliance::station_status_summary(&config, &report));
                Ok(())
            }
            Some(zeroclaw::StationCommands::Pause { worker_id }) => {
                appliance::pause_worker(&config, &worker_id).await
            }
            Some(zeroclaw::StationCommands::Resume { worker_id }) => {
                appliance::resume_worker(&config, &worker_id).await
            }
            Some(zeroclaw::StationCommands::MarkLoginComplete { worker_id }) => {
                appliance::mark_login_complete(&config, &worker_id).await
            }
            Some(zeroclaw::StationCommands::MarkChallengeComplete { worker_id }) => {
                appliance::mark_challenge_complete(&config, &worker_id).await
            }
            Some(zeroclaw::StationCommands::Start) | None => appliance::run_station(config).await,
        },
