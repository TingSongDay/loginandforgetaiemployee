...
pub struct Config {
    ...
    /// NeoHUman Station configuration (`[station]`).
    #[serde(default)]
...
/// NeoHUman Station target platform.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
#[serde(rename_all = "snake_case")]
pub enum StationPlatform {
    MacbookNeo,
}

impl Default for StationPlatform {
    fn default() -> Self {
        Self::MacbookNeo
    }
}

/// Fixed worker tile position inside the station layout.
#[derive(Debug, Clone, Copy, Serialize, Deserialize, JsonSchema, PartialEq, Eq, Hash)]
#[serde(rename_all = "snake_case")]
pub enum StationTilePosition {
    Left,
    Right,
}

/// Fixed browser settings for a NeoHUman Station worker.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct ManagedBrowserConfig {
    /// Headless mode should remain false for the visible station MVP.
    #[serde(default)]
    pub headless: bool,
    /// Optional absolute browser binary path for the managed browser.
    #[serde(default)]
    pub browser_binary_path: Option<String>,
    /// Fixed locale for browser rendering.
    #[serde(default = "default_station_browser_locale")]
    pub locale: String,
    /// Fixed timezone for browser rendering.
    #[serde(default = "default_station_browser_timezone")]
    pub timezone: String,
    /// Fixed user agent string.
    #[serde(default = "default_station_browser_user_agent")]
    pub user_agent: String,
    /// Fixed viewport width in pixels.
    #[serde(default = "default_station_viewport_width")]
    pub viewport_width: u32,
    /// Fixed viewport height in pixels.
    #[serde(default = "default_station_viewport_height")]
...
fn default_station_browser_timezone() -> String {
    "UTC".into()
}

fn default_station_browser_user_agent() -> String {
    "NeoHUmanStation/1.0".into()
}

fn default_station_viewport_width() -> u32 {
    960
}

fn default_station_viewport_height() -> u32 {
    900
}

fn default_station_zoom_percent() -> u16 {
    100
}
...
impl Default for ManagedBrowserConfig {
    fn default() -> Self {
        Self {
            ...
            locale: default_station_browser_locale(),
            timezone: default_station_browser_timezone(),
            user_agent: default_station_browser_user_agent(),
...
/// Per-worker configuration for NeoHUman Station.
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema, PartialEq, Eq)]
pub struct StationWorkerConfig {
    /// Stable worker identifier.
    pub id: String,
    /// Human-readable worker name.
    pub display_name: String,
    /// Whether this worker slot is enabled.
    #[serde(default = "default_true")]
    pub enabled: bool,
    /// Fixed tile assignment for the visible layout.
    pub tile_position: StationTilePosition,
    /// Browser profile name for this worker.
    pub profile_name: String,
    /// Workspace namespace for this worker.
    pub workspace_name: String,
    /// Relative dedupe state file path for this worker.
    pub dedupe_store_path: String,
    /// Fixed browser settings for this worker tile.
    #[serde(default)]
    pub managed_browser: ManagedBrowserConfig,
}

/// NeoHUman Station configuration (`[station]` section).
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct StationConfig {
    /// Enable NeoHUman Station runtime mode.
    #[serde(default)]
    pub enabled: bool,
    /// Branded station name.
    #[serde(default = "default_station_name")]
    pub station_name: String,
    /// Hardware target.
    #[serde(default)]
    pub platform: StationPlatform,
    /// How often each worker checks for new messages.
    #[serde(default = "default_station_message_poll_interval_ms")]
    pub message_poll_interval_ms: u64,
    /// Maximum time to wait for manual intervention before surfacing a blocked state.
    #[serde(default = "default_station_manual_intervention_timeout_secs")]
    pub manual_intervention_timeout_secs: u64,
    /// Reply engine mode. MVP defaults to deterministic reply logic.
    #[serde(default = "default_station_reply_mode")]
...
fn default_station_name() -> String {
    "NeoHUman Station".into()
}

fn default_station_message_poll_interval_ms() -> u64 {
    1_000
}

fn default_station_manual_intervention_timeout_secs() -> u64 {
    300
}

fn default_station_reply_mode() -> String {
    "deterministic".into()
}

fn default_station_workers() -> Vec<StationWorkerConfig> {
    vec![
        StationWorkerConfig {
            id: "worker_a".into(),
            display_name: "Worker A".into(),
            enabled: true,
            tile_position: StationTilePosition::Left,
            profile_name: "worker-a".into(),
            workspace_name: "worker-a".into(),
            dedupe_store_path: "state/worker-a-dedupe.json".into(),
            managed_browser: ManagedBrowserConfig {
                window_origin_x: 0,
                window_origin_y: 0,
        ...
        StationWorkerConfig {
            ...
            enabled: true,
            tile_position: StationTilePosition::Right,
            profile_name: "worker-b".into(),
            workspace_name: "worker-b".into(),
            dedupe_store_path: "state/worker-b-dedupe.json".into(),
            managed_browser: ManagedBrowserConfig {
                window_origin_x: default_station_viewport_width() as i32,
                window_origin_y: 0,
                ..ManagedBrowserConfig::default()
            },
        },
...
impl Default for StationConfig {
    fn default() -> Self {
        Self {
            ...
            platform: StationPlatform::default(),
            message_poll_interval_ms: default_station_message_poll_interval_ms(),
...
impl Config {
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.station.station_name.trim().is_empty() {
            anyhow::bail!("station.station_name must not be empty");
        }
        if self.station.message_poll_interval_ms == 0 {
            anyhow::bail!("station.message_poll_interval_ms must be greater than 0");
        }
        if self.station.manual_intervention_timeout_secs == 0 {
            anyhow::bail!("station.manual_intervention_timeout_secs must be greater than 0");
        }
        ...
        if self.station.enabled && self.station.workers.len() != 2 {
            anyhow::bail!(
                "station.enabled requires exactly 2 workers for the NeoHUman Station MVP"
            );
        }
        let mut seen_worker_ids = std::collections::HashSet::new();
        let mut seen_tiles = std::collections::HashSet::new();
        for (i, worker) in self.station.workers.iter().enumerate() {
            if worker.id.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].id must not be empty");
            }
            if worker.display_name.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].display_name must not be empty");
            }
            if worker.profile_name.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].profile_name must not be empty");
            }
            if worker.workspace_name.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].workspace_name must not be empty");
            }
            if worker.dedupe_store_path.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].dedupe_store_path must not be empty");
            }
            if worker.managed_browser.locale.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].managed_browser.locale must not be empty");
            }
            if worker.managed_browser.timezone.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].managed_browser.timezone must not be empty");
            }
            if worker.managed_browser.user_agent.trim().is_empty() {
                anyhow::bail!("station.workers[{i}].managed_browser.user_agent must not be empty");
            }
            if worker.managed_browser.viewport_width == 0 {
                anyhow::bail!(
                    "station.workers[{i}].managed_browser.viewport_width must be greater than 0"
                );
            }
            if worker.managed_browser.viewport_height == 0 {
                anyhow::bail!(
                    "station.workers[{i}].managed_browser.viewport_height must be greater than 0"
                );
            }
            ...
            {
                anyhow::bail!(
                    "station.workers[{i}].managed_browser.browser_binary_path must not be empty"
                );
            }
            if !seen_worker_ids.insert(worker.id.trim().to_string()) {
                anyhow::bail!("station.workers[{i}].id must be unique");
            }
            if !seen_tiles.insert(worker.tile_position.clone()) {
                anyhow::bail!("station.workers[{i}].tile_position must be unique");
            }
        }
        if self.station.enabled
            && (!seen_tiles.contains(&StationTilePosition::Left)
                || !seen_tiles.contains(&StationTilePosition::Right))
        {
            anyhow::bail!("station.enabled requires one left worker and one right worker");
        }
