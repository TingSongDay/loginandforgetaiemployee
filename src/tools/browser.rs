...
pub struct BrowserTool {
    ...
    launch_options: Option<ManagedBrowserLaunchOptions>,
    backend: String,
...
/// Worker-scoped launch configuration for managed browser sessions.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct ManagedBrowserLaunchOptions {
...
impl BrowserTool {
    pub fn new(
        ...
        Self::new_with_backend_and_launch_options(
            security,
            allowed_domains,
            session_name,
            None,
    ...
    #[allow(clippy::too_many_arguments)]
    pub fn new_with_backend_and_launch_options(
        security: Arc<SecurityPolicy>,
        allowed_domains: Vec<String>,
        session_name: Option<String>,
        launch_options: Option<ManagedBrowserLaunchOptions>,
    ...
    pub fn launch_options(&self) -> Option<&ManagedBrowserLaunchOptions> {
    ...
    /// Execute a structured browser action without going through JSON tool arguments.
    pub async fn execute_structured_action(
        &self,
        action: BrowserAction,
    ) -> anyhow::Result<ToolResult> {
    ...
    /// Ensure the configured backend has a live managed browser session.
    pub async fn launch_managed_session(&self) -> anyhow::Result<()> {
        let backend = self.resolve_backend().await?;
        match backend {
...
mod native_backend {
    ...
    impl NativeBrowserState {
        ...
        pub async fn execute_action(
            ...
            match action {
                BrowserAction::Open { url } => {
                    self.ensure_session(headless, webdriver_url, chrome_path, launch_options)
                        .await?;
        ...
        async fn ensure_session(
            ...
            let launch = launch_options
                .cloned()
                .unwrap_or_else(|| ManagedBrowserLaunchOptions {
                    session_name: "zeroclaw".to_string(),
                    headless,
                    browser_binary_path: chrome_path.map(ToOwned::to_owned),
                    user_data_dir: std::path::PathBuf::new(),
                    locale: "en-US".to_string(),
                    timezone: "UTC".to_string(),
                    user_agent: "ZeroClaw/1.0".to_string(),
                    viewport_width: 1280,
                    viewport_height: 900,
                    window_origin_x: 0,
                    window_origin_y: 0,
            ...
            if !launch.user_data_dir.as_os_str().is_empty() {
            ...
            args.push(Value::String(format!(
                "--window-size={},{}",
                launch.viewport_width, launch.viewport_height
            )));
            args.push(Value::String(format!(
                "--window-position={},{}",
                launch.window_origin_x, launch.window_origin_y
            )));
            if !launch.locale.trim().is_empty() {
                args.push(Value::String(format!("--lang={}", launch.locale.trim())));
            }
            if !launch.user_agent.trim().is_empty() {
                args.push(Value::String(format!(
                    "--user-agent={}",
                    launch.user_agent.trim()
                )));
            }
            if launch.zoom_percent != 100 {
                let scale = f64::from(launch.zoom_percent) / 100.0;
                args.push(Value::String(format!(
            ...
            if !launch.locale.trim().is_empty() {
                prefs.insert(
                    ...
                    Value::String(launch.locale.clone()),
                );
            }
            ...
            if !prefs.is_empty() {
                chrome_options.insert("prefs".to_string(), Value::Object(prefs));
            }

            if let Some(path) = launch.browser_binary_path.as_deref().or(chrome_path) {
...
(() => {
  try {
    ...
    if (document && document.body && arguments[5] !== "100%") {
...
"#,
                    vec![
                        json!(launch.window_origin_x),
                        json!(launch.window_origin_y),
                        json!(launch.viewport_width),
                        json!(launch.viewport_height),
                        json!(launch.locale),
                        json!(format!("{}%", launch.zoom_percent)),
                    ],
                )
                .await;
            Ok(())
