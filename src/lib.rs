...
/// NeoHUman Station management subcommands
#[derive(Subcommand, Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub enum StationCommands {
    /// Start the NeoHUman Station supervisor runtime
    Start,
    /// Print the configured worker layout and station state
    Status,
    /// Pause one worker without closing its browser session
    Pause {
        /// Stable station worker id
        worker_id: String,
    },
    /// Resume one paused worker
    Resume {
        /// Stable station worker id
        worker_id: String,
    },
    /// Mark manual login as complete for one worker
    MarkLoginComplete {
        /// Stable station worker id
        worker_id: String,
    },
    /// Mark manual challenge handling as complete for one worker
